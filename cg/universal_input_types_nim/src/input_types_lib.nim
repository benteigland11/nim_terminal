...
##
## This widget provides the basic enums and objects for describing
## user input (keyboard, mouse) in a platform-neutral way. Both
## encoders (writing to PTY) and listeners (reading from terminal)
## can use these types to share a common vocabulary.
...
type
  ...
  MouseEvent* = object
    ...
    button*: MouseButton
    row*, col*: int
    xPixel*, yPixel*: int  ## Optional pixel-precision for SGR-pixel protocol
    mods*: set[Modifier]
