...
**Use widgets**
    ...
    upgrade <widget_id> [--target .] [--version VERSION]
      Upgrade an installed widget to the latest version

    status [widget_id] [--target .] [--page 1] [--size 20] [--all]
      Check installed widget(s) - omit widget_id to scan all

    rate <widget_id> <score> [--comment COMMENT] [--target .]
      Rate a widget (local or @handle/widget-id for cloud)


**Build widgets**

    create <widget_id> [--language angular|javascript|nim|php|python|typescript] [--domain backend|data|frontend|infra|ml|modeling|rtl|security|universal] [--name NAME] [--target .]
    ...
    rename <widget_id> [--name NAME] [--domain backend|data|frontend|infra|ml|modeling|rtl|security|universal] [--target .]
      Rename a widget's slug or domain (local only; cloud copy untouched)
